...
BASE_PATH="/srv"
REQUIRED_PACKAGES=("python3" "python3-pip" "acl" "curl" "wget" "htop" "ufw")
mkdir -p /tmp/log
touch "$LOG_FILE"
...
# Directory structure, permissions, ACLs
setup_directories() {
  log_info "Creating base directory tree..."
  mkdir -p "$BASE_PATH"
  chown "$USER:$USER" "$BASE_PATH"
  ...
  log_info "Applying ACLs..."
  setfacl -R -m u::rwx,g::rwx,o::rx "$BASE_PATH"
  setfacl -R -d -m u::rwx,g::rwx,o::rx "$BASE_PATH"
...
setup_firewall() {
  ...
  ufw allow 2220      comment "SSH"
  ufw allow 2222/tcp comment "SFTP"
